...
5. 두 스레드가 서로 다른 키를 각각 put 한 번씩 넣어도 왜 삽입이 유실될까?
...
### 두 스레드가 put하면: 버킷과 충돌 처리에서 삽입이 유실되는 흐름

앞의 A 예제는 두 put을 **순서대로** 호출했다. 이제 같은 HashMap에 두 스레드가
각각 `put` 한 번씩 호출한다고 하자. 사용자 코드에 `get + put`이 없어도 문제가 생긴다.
`put → hash → putVal` 내부의 확인과 저장 사이에 다른 스레드가 끼어들 수 있기 때문이다.

아래는 위 OpenJDK 21 코드에서 가능한 실행 순서를 손으로 추적하는 연습이다.
실제 HashMap을 반복 실행해서 반드시 같은 실패가 나온다는 뜻은 아니다.
배열은 작업 스레드 시작 전에 이미 생성·공유되었고, 이 삽입에서는 resize나 트리화가
발생하지 않는다고 가정한다. A와 B는 각각 `HashCodeCollisionKey(1)`, `(2)`다.
둘 다 hash=7이지만 equals는 false인 정상적인 서로 다른 키다.

#### 1. 빈 버킷: 두 스레드가 같은 배열 칸을 덮어쓴다

배열 길이가 16이면 두 키 모두 `(16 - 1) & 7 = 7`이므로 `table[7]`을 선택한다.
처음에 이 칸이 null인 경우를 보자.

```java
if ((p = tab[i = (n - 1) & hash]) == null) // ① 읽고 빈 칸인지 확인
    tab[i] = newNode(hash, key, value, null); // ② 새 노드 참조 저장
```

| 순서 | 스레드 T1: put(A, "a") | 스레드 T2: put(B, "b") |
| --- | --- | --- |
| 1 | ①에서 table[7]이 null임을 확인 | |
| 2 | | ①에서 table[7]이 null임을 확인 |
| 3 | ②에서 table[7]에 A 노드 저장 | |
| 4 | | ②에서 table[7]에 B 노드 저장 |

```text
처음:             table[7] → null
T1 저장 직후:     table[7] → A → null
T2 저장 직후:     table[7] → B → null   (A에 도달하는 연결이 사라짐)
순서대로 했다면:  table[7] → A → B → null
```

T2는 이미 빈 버킷 분기를 선택했으므로 저장 직전에 null인지 다시 확인하지 않는다.
따라서 T1이 넣은 A를 발견해 충돌 처리 분기로 들어가지 않고 B로 덮어쓴다.
**충돌 처리 코드가 있어도, 버킷을 확인하고 수정하는 과정이 동기화되어 있지는 않다.**
각 호출의 `tab`, `i`, `p`는 지역 변수지만, `tab`이 가리키는 배열은 같은 공유 객체다.
참조 하나의 대입이 원자적이어도 “확인 후 저장” 전체가 원자적인 것은 아니다.

#### 2. 이미 충돌 리스트가 있어도: 같은 next를 덮어쓴다

이번에는 `table[7] → P → null`이 초기 상태이고 P도 hash=7인 다른 키라고 하자.
두 스레드는 P와 키가 다르다는 비교를 통과하고 일반 리스트 분기로 들어간다.

```java
if ((e = p.next) == null) { // ① 현재 노드가 리스트 끝인지 확인
    p.next = newNode(hash, key, value, null); // ② 끝에 새 노드 연결
    // 트리화 조건 검사 생략
    break;
}
```
...
| 순서 | 스레드 T1 | 스레드 T2 |
| --- | --- | --- |
| 1 | p=P, P.next가 null임을 확인 | |
| 2 | | p=P, P.next가 null임을 확인 |
| 3 | P.next에 A 노드 저장 | |
| 4 | | P.next에 B 노드 저장 |

```text
실제 가능한 연결: table[7] → P → B → null   (A 연결 유실)
순서대로 했다면:  table[7] → P → A → B → null
```

빈 버킷에서는 `tab[i]`, 리스트 끝에서는 `p.next`라는 공유 위치를 두 스레드가 덮어쓴다.
hash 충돌은 같은 버킷을 선택하는 현상이고, 이 race condition은 그 버킷의 연결을
조율 없이 수정하는 문제다. **충돌 자체가 데이터 유실을 뜻하지는 않는다.**
또한 hashCode가 달라도 버킷 인덱스가 같을 수 있으므로 동일 hashCode만의 문제도 아니다.

#### 3. 버킷이 다르면 안전할까? size도 공유 상태다

서로 다른 버킷에 노드를 정상적으로 넣었더라도 두 호출은 마지막에 같은 `size`를 수정한다.
...
`++size`는 읽기·증가·쓰기를 하나의 원자적 작업으로 보장하지 않는다.
둘 다 기존 size=5를 읽고 6을 쓰면 실제 엔트리는 7개인데 size는 6이 될 수 있다.
앞의 연결 유실과는 별개의 경쟁이며, size가 맞다는 사실만으로 연결이 온전하다고
판정할 수도 없다. `modCount`의 증가 역시 동기화를 제공하지 않는다.

resize에서는 배열 참조와 노드 연결까지 변경한다. 따라서 충돌을 줄이거나 초기 용량을
크게 잡아도 HashMap을 thread-safe하게 만들 수 없다. 여기서는 resize 경쟁까지
추적하기 전에, **resize가 없어도 put 내부에서 이미 경쟁한다**는 점을 먼저 설명한다.
동시 쓰기와 읽기 사이에는 변경의 가시성 문제도 있다. 기존 키의 value만 바꾸는 경우도
별도의 동기화 없이 안전하다고 추론하면 안 된다.

#### 4. 이 문제를 해결한 뒤 get + put으로 넘어간다

| 학습 단계 | 끼어드는 위치 | 필요한 보장 |
| --- | --- | --- |
| 일반 HashMap에 각각 put | putVal 내부의 버킷 확인과 연결 수정 사이 | 내부 공유 상태를 안전하게 읽고 수정 |
| thread-safe한 Map에서 get 후 put | 두 메서드 호출 사이 | 읽기·계산·쓰기 전체의 원자성 |
| 두 키를 각각 compute | 두 compute 호출 사이 | 두 키 변경과 관찰을 묶는 경계 |

`synchronizedMap`은 동일한 잠금으로 각 메서드 실행을 보호한다. 모든 접근을 래퍼로
통일하면 위의 두 put 내부 실행은 서로 끼어들지 않는다. ConcurrentHashMap은
빈 버킷에 CAS로 노드를 설치하고 기존 일반 버킷에서는 잠금과 재확인으로 삽입을 조율한다.
하지만 그 보장만으로 사용자가 호출하는 `get + put` 전체가 원자적이 되지는 않는다.
다음 [동시성 단원](../concurrency/README.md)에서 이 두 번째 문제를 실험한다.

**확인 질문:** 두 스레드가 모두 null을 읽은 지점에서 멈췄다고 가정하고, 위 코드의
다음 대입을 T1, T2 순서로 실행해 배열과 next 연결을 그려 보자. A를 다시 찾으려고
`get(A)`를 호출하면 어느 버킷에서 어떤 연결을 따라가게 될까?
...
## 9. 캐시와 트랜잭션으로 가져갈 네 가지

### 키의 동등성 → 캐시 키 설계

예를 들어 테넌트별 상품 정보를 캐시한다면 다음처럼 결과를 식별하는 정보를 키에 포함한다.
아래는 설계 예시이며 Demo에 추가된 실행 코드는 아니다.

```java
record ProductKey(String tenantId, long productId) {}
// 두 필드로 equals/hashCode를 함께 생성한다.
// String과 long만 사용하므로 비교에 쓰는 상태를 삽입 뒤 바꿀 수 없다.

var a = new ProductKey("tenant-A", 9);
var b = new ProductKey("tenant-A", 9);
var c = new ProductKey("tenant-B", 9);
// a.equals(b) → true: 같은 데이터를 조회하는 키
// a.equals(c) → false: 상품 번호가 같아도 테넌트가 다르면 다른 데이터
```

productId만 키로 쓰면 서로 다른 테넌트의 데이터를 같은 캐시 엔트리로 취급할 수 있다.
결과가 언어 등 다른 조건에도 의존한다면 그 조건도 키 설계에 반영해야 한다.
record라는 문법만으로 모든 객체가 불변이 되는 것은 아니다. 가변 List 등을 필드로 넣고
내용을 바꾸면 equals/hashCode에 영향을 줄 수 있다.

우리 `MutableKey` 실험은 삽입 뒤 상태를 바꾸면 조회가 실패함을 보여 준다.
같은 탐색 원리에 의존하는 삭제·캐시 무효화도 영향을 받을 수 있다.
`HashCodeEqualsCollisionKey`는 새 객체로도 동등 키를 찾아 값을 교체하는 정상 경로다.
`EqualsCollisionKey`는 그 계약을 어기면 어떻게 되는지를 보여 주는 반례다.

### 해시 분산 → 캐시 조회 비용

| 관찰 | 캐시에서의 의미 |
| --- | --- |
| 다른 키의 hash가 같아도 equals로 구별 | 충돌 자체가 잘못된 값 반환을 뜻하지는 않는다. |
| 모든 hash가 7이면 확장해도 한 버킷에 남음 | hashCode 품질이 나쁘면 빈 버킷이 많아도 조회가 느려질 수 있다. |
| 동일 hash·비교 불가능한 키는 트리의 양쪽을 탐색할 수 있음 | 트리화만으로 모든 조회의 로그 시간을 보장하지 않는다. |

충돌은 “키를 어떻게 찾을까”의 문제다. LRU의 퇴출은 “어떤 엔트리를 남길까”의 문제다.
HashMap의 resize는 배열을 키우며 기존 매핑을 유지한다.
캐시의 용량 제한은 필요할 때 엔트리를 제거하므로 두 동작을 구분한다.
다음 [cache 문서](../cache/README.md)에서 이 차이를 실제 예제와 연결한다.

### 메모리 저장 → 재생성과 복구의 차이

```text
원본 DB → 조회 결과를 메모리 캐시에 보관
프로세스 종료 → 캐시 유실 → 원본에서 다시 읽을 수 있음

메모리 Map만 유일한 잔액 저장소로 사용
프로세스 종료 → 잔액 유실 → 복구할 원본이 없음
```

첫 번째 구조는 캐시 미스 때 원본을 다시 읽는 비용과 데이터의 최신성을 다뤄야 한다.
두 번째 구조는 성공했다고 응답한 변경을 어떻게 복구할지 먼저 정해야 한다.
이 프로젝트의 LRU/Caffeine은 메모리 캐시이며,
[storage.recovery](../storage/recovery/README.md)는 WAL을 복구 기준으로 추가한다.
“메모리에 put이 끝남”과 “장애 후에도 커밋이 남음”은 다른 보장이다.

### 한 키 교체 → 복합 연산과 두 키 이체

아래는 문제를 설명하기 위한 코드다. 일반 HashMap을 여러 스레드가 함께 수정하는 예제로 실행하지 않는다.

```java
int current = map.get("count"); // 읽기
map.put("count", current + 1);  // 계산 결과 쓰기: 두 호출 사이에 다른 작업이 끼어들 수 있음
```

다음 [concurrency](../concurrency/README.md)는 스레드 안전한 Map으로 바꿔도
get과 put 사이에 개입이 가능해 증가가 유실되는 것을 재현한다.
ConcurrentHashMap.compute로 한 키의 증가를 묶은 다음에는 두 키로 범위를 넓힌다.

```text
초기: A=1000, B=1000, 합계=2000
A 차감: A=900, B=1000, 합계=1900  ← 독자가 보면? 여기서 실패하면?
B 증가: A=900, B=1100, 합계=2000
```

[storage.atomic](../storage/atomic/README.md)은 두 키의 변경과 snapshot을 같은 잠금으로
묶고, 처리 가능한 예외가 나면 이전 값을 복구한다. HashMap.put의 값 교체 자체를
스레드 안전한 원자 연산이나 DB 커밋으로 해석하면 안 된다.

### 여기서 다음 단계로 넘어가는 기준

1. 동등한 새 키 객체로 조회할 수 있지만, 삽입한 키의 상태를 바꾸면 왜 실패하는가?
2. resize가 충돌을 항상 없애지 못하는 이유는 무엇인가?
3. 메모리 캐시를 잃는 것과 유일한 잔액 저장소를 잃는 것은 어떻게 다른가?
4. 한 키의 값 교체가 왜 두 키 이체의 원자성을 보장하지 않는가?
5. 동시에 실행하는 두 put이 빈 버킷과 충돌 리스트에서 각각 어떤 공유 위치를 덮어쓰는가?
6. 서로 다른 버킷에 넣더라도 size가 틀어질 수 있는 이유는 무엇인가?

이 여섯 가지를 설명할 수 있으면 내부 트리 심화는 멈추고 concurrency로 넘어간다.
