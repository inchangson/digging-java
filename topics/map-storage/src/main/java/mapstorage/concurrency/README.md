...
## 시작점: HashMap 내부 경쟁과 복합 연산 경쟁을 구분하기

먼저 [HashMap의 put 내부 경쟁](../hashmap/README.md#두-스레드가-put하면-버킷과-충돌-처리에서-삽입이-유실되는-흐름)을 읽는다.
일반 HashMap에서는 각 스레드가 put 한 번만 호출해도 `tab[i]`나 `p.next`를
덮어써 서로 다른 키의 삽입이 유실될 수 있다. 서로 다른 버킷이어도 `++size`가 경쟁한다.

이 단원의 CounterDemo는 그 내부 경쟁을 재현하는 실험이 아니다.
각 메서드의 스레드 안전성이 확보된 Map을 사용한 뒤에도 **get과 put 사이**에
다른 스레드가 끼어들 수 있다는 다음 단계의 문제를 재현한다.
실행 전에 삽입 유실과 증가 유실의 차이, 각각 보호해야 할 범위를 설명해 보자.
